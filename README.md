...
[![Version](https://img.shields.io/badge/version-v2.38.0-green)](https://github.com/Nairon-AI/flux/releases)
...
- **Anti-sycophancy built in** — Flux will tell you "this isn't worth building" or "you're making assumptions you haven't validated." During scoping, it runs a viability gate, stress-tests one-way door decisions, and applies a future-pressure pass so likely follow-on features, failure modes, and reversal costs are examined *before* any code is written
...
| **Future Pressure** | *Auto during Scope/Plan:* quick forecast on every plan, deeper pass for one-way doors and shared surfaces. Writes likely follow-on features, failure modes, observability needs, and reversal triggers into the epic/task specs. | A plan can be locally correct and still be wrong for the system six weeks later. Future Pressure is Flux's slop-creep circuit breaker: it asks what happens when the next features arrive, traffic grows, or the abstraction proves wrong. That forecast belongs before implementation, not as a month-end apology tour. |
...
| **Meditate** | *Auto after Reflect (conditional):* prunes stale notes, promotes pitfalls to principles, and runs a periodic future-pressure audit. Triggers when 20+ pitfall files accumulate. | The brain vault grows without bound. Old pitfalls become irrelevant (code was refactored), recurring patterns deserve promotion to principles (stronger signal), and repeated forecast misses should become tighter guardrails. Without periodic curation, the brain becomes noise — too many files, contradictory advice, stale warnings about code that no longer exists. |
...
This is the corrective layer, not the primary defense. Flux now treats **future pressure** during scope/plan as the prevention layer: feature-by-feature forecasting for one-way doors and shared surfaces. `desloppify` and `meditate` handle the "what still slipped through?" side later.
