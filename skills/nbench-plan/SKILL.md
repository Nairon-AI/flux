---
name: nbench-plan
description: Create structured build plans from feature requests or Flow IDs. Use when planning features or designing implementation. Triggers on /nbench:plan with text descriptions or Flow IDs (fn-1-add-oauth, fn-1-add-oauth.2, or legacy fn-1, fn-1.2, fn-1-xxx, fn-1-xxx.2).
...
**Task size**: every task must fit one `/nbench:work` iteration (~100k tokens max). If it won't, split it.
...
**Why:** Implementation happens in `/nbench:work` with fresh context. Writing it here wastes tokens in planning, review, AND implementation — then causes drift when the implementer does it differently anyway.
...
- Chained instructions like "then review with /nbench:plan-review"

Examples:
- `/nbench:plan Add OAuth login for users`
- `/nbench:plan fn-1-add-oauth`
- `/nbench:plan fn-1` (legacy formats fn-1, fn-1-xxx still supported)
- `/nbench:plan fn-1-add-oauth then review via /nbench:plan-review`
...
- Option 2a: run `/nbench:plan-review` after Step 4, fix issues until it passes
- Option 2b: run `/nbench:plan-review` with export mode after Step 4
