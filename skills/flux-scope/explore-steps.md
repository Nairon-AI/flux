# Explore Mode — Detailed Execution Steps

Step-by-step execution guide for explore mode (`--explore`). For architecture overview, see [explore.md](explore.md). For approach patterns, see [approaches.md](approaches.md).
...
# 3. Generate preview for hybrid (same as Step 6.8)
# 4. Present updated comparison (including hybrid) — return to Step 6.9
```
...
**Workflow:** Spawn agent to refine in existing worktree, regenerate preview, return to Step 6.9 with updated comparison.
...
$FLUXCTL explore set-decision <epic-id> \
  --selected "a" \
  --rationale "Modal wizard chosen for familiar UX pattern." \
  --rejected "b:too limited for complex forms,c:screen space concerns" \
  --json
```
