...
| 4. Deliver | Install to `.codex/skills/<name>/` with a legacy `.claude/skills/` mirror, generate trigger test report | Installed and ready |
...
- **Validate the install path, not the repo path.** Run `validate_skills.py` against `.codex/skills/<name>/` (where the skill is installed), not `skills/<name>/` (which is the Flux plugin's own skills directory).
