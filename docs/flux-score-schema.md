...
Flux measures how effectively an engineer works with AI coding agents. The score is computed from observable session data—not self-reported surveys or vibes.
...
percentile: 89  # vs. other Flux users
```
...
flux export --period 30d --format yaml > evidence.yaml

# Compare against benchmarks
flux compare --percentile
```
